---
title: "Release Notes of the twinsphere Cloud Platform"
---
...
- **[Deprecation]** Removed obsolete, superseded features:
    - OAuth2 **machine clients** — replaced by [service accounts](/id-service-accounts) managed via the Manage API
...
- **[Deprecation]** Filter Queries (`POST /filter/shells`) are deprecated — please use the AAS
    [Query Language](/cloud-documentation#query-language) instead
...
- **[ManageUI]** New twinsphere **Manage UI** — a web interface for organization-level data: users, groups,
    service accounts and role assignment, see [Management](/management-overview)
...
- **[PushService]** SAP Business Network Asset Collaboration (SAP BNAC) - support for Handover Documentation V2.0
    and sharing equipment via BNAC Authorization Groups, see [Push Service](/cloud-push-service) for more information
...
- **[API]** AAS 3.1 [Query Language](/cloud-documentation#query-language) — structured JSON queries
...
- **[PushService]** SAP Business Network Asset Collaboration (SAP BNAC) push target (experimental),
    see [Push Service](/cloud-push-service#sap-bnac-push-jobs-experimental) for more information
...
- **[SphereAPI]** Introduced Statistics API endpoint for retrieving historical timeseries data about statistics,
    see [Statistics](/cloud-statistics) for more information
...
- **[Security]** Introduced [Attribute-Based Access Control (ABAC)](/cloud-abac) for fine-grained authorization
...
- **[ManageAPI]** Added [User registration](/id-registration)
...
- **[Files]** Added [File Filter Query API](/cloud-file-filter-queries) which leverages the recently introduced
...
- **[ManageAPI]** Introduced [user groups](/management-overview) for more convenient user management
- **[Bugfix]** Fixed idShort path resolving for list elements

:::caution[File endpoints have moved]
...
:::caution[Deprecation of Basic Auth]
Our support of Basic Auth will end with this calendar year.  
The reason is that twinsphere's authorization model is based on [twinsphere ID](/id-overview) and does not
support Basic Auth.  
...
- **[Filter]** Added further shell properties displayName, idShort and aas-id to [Filter Queries](/cloud-shell-filter-queries)
...
- **[Filter]** Introduction of Filter Queries feature, see [documentation](/cloud-shell-filter-queries) for more information
...
- **[SemConn]** Introduction of Semantic Connector feature, see [documentation](/cloud-semantic-connector) for more
  information
...
- **[API]** Extended the SM repository with an [optional kind filter](/cloud-documentation#enhanced-get-submodels)
...
- **[Docs]** Added [technical documentation on twinsphere ID](/cloud-auth)
- **[Docs]** Documented reason and workaround for creation of invalid package files by [AASX
  serialization](/cloud-documentation#serialization)
...
- Gzip compression of the message payload of the change events
    ...
    - For usage and notes see [Events](/cloud-events#topics-message-contracts-and-qos)
...
- Event mechanism for cyclical information about changes to AAS & SM (experimental)
    - For setup and usage please refer to [Events](/cloud-events)
