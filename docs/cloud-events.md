...
- Events are best effort. They are not retried and not replayed, so an event can be missed — while your session is
  disconnected, while the broker is unreachable, or because your session was terminated for falling behind. Do not use
  events as a system of record; use them as a prompt to read current state.
...
A disconnect that happens only **under load** and reconnects cleanly is usually not a configuration problem, but your
session being dropped for falling behind: unread messages pile up until the broker terminates it. What decides this is
your own consumption rate, not the 100 per second ceiling — at QoS 1 most libraries acknowledge when your handler
returns, so a handler taking 200 ms consumes five messages per second. Acknowledge first and process on separate
workers. A persistent session does not help, as its offline queue holds only 100 messages. MQTT 3.1.1 gives no reason
for a disconnect, so this looks like a network fault; connect with **MQTT v5** and the broker states it
(`QuotaExceeded`).
